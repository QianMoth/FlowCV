            ...
            if (!json_data.empty()) {
                if (json_data.contains("data")) {
                    for (auto &d : json_data["data"]) {
                        if (counting_mode_ == 0) {
                            if (d.contains("size")) {
                                int shapeSize = static_cast<int>(d["size"].get<float>());
                                if (shapeSize > (size_match_ - size_threshold_) &&
                                    shapeSize < (size_match_ + size_threshold_)) {
                                    sizeCnt++;
                                    entry["count"] = sizeCnt;
                                }
                            }
                    ...
                    if (show_overlay_) {
                        if (counting_mode_ == 0) {  // By Size
                            ...
                            cv::Size txtSize = cv::getTextSize(countStr, text_font_, text_scale_,
                                                               text_thickness_, &baseline);
                            int roiHeight = ((txtSize.height + txtOffset) * 2) + 10;
                            int txtPosHeightOffset = static_cast<int>(txtSize.height) + 10;
                            cv::Rect roiRect =
                                cv::Rect(text_pos_.x - 10, text_pos_.y - txtPosHeightOffset,
                                         txtSize.width + 20, roiHeight);
                            if (roiRect.x < 0) {
                                roiRect.x = 0;
                                text_pos_.x = 10;
                            }
                            ...
                            if (roiRect.x + roiRect.width > frame.cols) {
                                roiRect.x = ((frame.cols - roiRect.width) >= 0)
                                                ? (frame.cols - roiRect.width)
                                                : 0;
                                roiRect.width = (frame.cols - roiRect.x) - 1;
                                text_pos_.x = roiRect.x + 10;
                            }
                            if (roiRect.y + roiRect.height > frame.rows) {
                                roiRect.y = ((frame.rows - roiRect.height) >= 0)
                                                ? (frame.rows - roiRect.height)
                                                : 0;
                            ...
                            cv::putText(frame, sizeStr, cv::Point(text_pos_.x, text_pos_.y),
                                        text_font_, text_scale_,
                                        cv::Scalar(text_color_.z * 255, text_color_.y * 255,
                                                   text_color_.x * 255),
                                        text_thickness_);
                            cv::putText(
                                frame, countStr,
                                cv::Point(text_pos_.x, text_pos_.y + txtSize.height + txtOffset),
                                text_font_, text_scale_,
                                cv::Scalar(text_color_.z * 255, text_color_.y * 255,
                                           text_color_.x * 255),
                                text_thickness_);
                            ...
                            if (!classCount.empty()) {
                                ...
                                cv::Size txtSize =
                                    cv::getTextSize(placeholder, text_font_, text_scale_,
                                                    text_thickness_, &baseline);
                                int roiHeight = ((txtSize.height + txtOffset) *
                                                 static_cast<int>(classCount.size())) +
                                                15;
                                int txtPosHeightOffset = static_cast<int>(txtSize.height) + 10;
                                cv::Rect roiRect =
                                    cv::Rect(text_pos_.x - 10, text_pos_.y - txtPosHeightOffset,
                                             txtSize.width, roiHeight);
                                if (roiRect.x < 0) {
                                    roiRect.x = 0;
                                    text_pos_.x = 10;
                                }
                                ...
                                for (auto &cnt : classCount) {
                                    ...
                                    cv::putText(frame, sizeStr, cv::Point(text_pos_.x, startY),
                                                text_font_, text_scale_,
                                                cv::Scalar(text_color_.z * 255, text_color_.y * 255,
                                                           text_color_.x * 255),
                                                text_thickness_);
...
void ShapeCounter::UpdateGui(void *context, int interface)
{
    ...
    if (interface == static_cast<int>(FlowCV::GuiInterfaceType_Controls)) {
        ...
        if (ImGui::DragInt(CreateControlString("X", GetInstanceName()).c_str(), &text_pos_.x,
                           0.5F)) {
            if (text_pos_.x < 0)
                text_pos_.x = 0;
        }
        ImGui::SameLine();
        ImGui::SetNextItemWidth(80);
        if (ImGui::DragInt(CreateControlString("Y", GetInstanceName()).c_str(), &text_pos_.y,
                           0.5F)) {
            if (text_pos_.y < 0)
                text_pos_.y = 0;
        }
        ...
        ImGui::Separator();
        ImGui::ColorEdit4(CreateControlString("Color", GetInstanceName()).c_str(),
                          (float *)&text_color_);
        ImGui::Separator();
        ImGui::Combo(CreateControlString("Counting Mode", GetInstanceName()).c_str(),
                     &counting_mode_, "By Size\0By ID\0\0");
        ImGui::Checkbox(CreateControlString("Show Overlay", GetInstanceName()).c_str(),
                        &show_overlay_);
        if (counting_mode_ == 0) {
            ImGui::SetNextItemWidth(80);
            ImGui::DragInt(CreateControlString("Size Match", GetInstanceName()).c_str(),
                           &size_match_, 0.5F, 1, 1000);
            ImGui::SetNextItemWidth(80);
            ImGui::DragInt(CreateControlString("Size Threshold", GetInstanceName()).c_str(),
                           &size_threshold_, 0.5F, 0, 1000);
        }
    }
}
