...
DiscreteFourierTransform::DiscreteFourierTransform() : Component(ProcessOrder::OutOfOrder)
{
    // Name and Category
    ...
    if (!in1->empty()) {
        if (IsEnabled()) {
            ...
            cv::dft(complexI, complexI,
                    dft_mode);  // this way the result may fit in the source matrix
...
void DiscreteFourierTransform::UpdateGui(void *context, int interface)
{
    ...
    if (interface == (int)FlowCV::GuiInterfaceType_Controls) {
        ...
        ImGui::Checkbox(CreateControlString("Shift DC to Center", GetInstanceName()).c_str(),
                        &shift_dc_center_);
        ImGui::Checkbox(CreateControlString("Logarithmic View", GetInstanceName()).c_str(),
                        &log_view_);
    }
}
