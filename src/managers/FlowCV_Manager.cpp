...
bool FlowCV_Manager::GetNodeInfoByIndex(uint64_t index, NodeInfo &nInfo)
{
    if (index >= 0 && index < nodes_.size()) {
        ...
        nInfo.input_conn_map.assign(nodes_.at(index).input_conn_map.begin(),
                                    nodes_.at(index).input_conn_map.end());
...
nlohmann::json FlowCV_Manager::GetState()
{
    ...
    for (const auto &node : nodes_) {
        ...
        if (!state_str.empty()) {
            nlohmann::json node_state = nlohmann::json::parse(state_str);
            n["params"] = node_state;
        }
        nodes.push_back(n);
    }
        ...
        if (state.contains("nodes")) {
            for (const auto &node : state["nodes"]) {
                ...
                if (plugin_manager_->HasPlugin(name.c_str())) {
                    ...
                    CheckInstCountValue(ni);
                    // std::cout << "Adding Node Instance (Plugin): " << name << ", " << id <<
                    // std::endl;
                    ...
                    if (internal_node_manager_->HasNode(name.c_str())) {
                        ...
                        ni.node_ptr->SetEnabled(enabled);
                        CheckInstCountValue(ni);
                        // std::cout << "Adding Node Instance (Internal): " << name << ", " << id <<
                        // std::endl;
                        if (node.contains("params")) {
                            ni.node_ptr->SetState(node["params"].dump());
                        }
...
{
    if (index >= 0 && index < nodes_.size()) {
        return nodes_.at(index).node_ptr->HasGui(static_cast<int>(interface));
    }
...
{
    if (index >= 0 && index < nodes_.size()) {
        return nodes_.at(index).node_ptr->UpdateGui(context, static_cast<int>(interface));
    }
}

bool FlowCV_Manager::ConnectNodes(uint64_t from_id, uint32_t from_out_idx, uint64_t to_id,
                                  uint32_t to_in_idx)
{
    bool res = false;
    ...
    if (from_id != 0 && to_id != 0) {
        res = circuit_->ConnectOutToIn(nodes_.at(from_index).node_ptr, from_out_idx,
                                       nodes_.at(to_index).node_ptr, to_in_idx);
        if (res) {
...
bool FlowCV_Manager::DisconnectNodes(uint64_t from_id, uint32_t from_out_idx, uint64_t to_id,
                                     uint32_t to_in_idx)
{
    for (int i = 0; i < wiring_.size(); i++) {
        if (wiring_.at(i).from.id == from_id && wiring_.at(i).from.index == from_out_idx &&
            wiring_.at(i).to.id == to_id && wiring_.at(i).to.index == to_in_idx) {
            wiring_.erase(wiring_.begin() + i);
            uint64_t node_idx = GetNodeIndexFromId(to_id);
            nodes_.at(node_idx).node_ptr->DisconnectInput(static_cast<int>(to_in_idx));
            return true;
        }
    }
        ...
        if (node_id != 0 && in_index >= 0 && in_index < ni.desc.input_count) {
            ...
            while (it != wiring_.end()) {
                if (it->to.id == node_id && it->to.index == in_index) {
                    ...
                    nodes_.at(node_idx).node_ptr->DisconnectInput(static_cast<int>(in_index));
...
{
    return static_cast<uint64_t>(wiring_.size());
}
