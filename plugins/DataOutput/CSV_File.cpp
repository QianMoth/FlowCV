...
CsvFile::CsvFile() : Component(ProcessOrder::OutOfOrder), p(new internal::CsvFile())
{
    ...
    // 1 inputs
    SetInputCount_(3, {"start", "frame", "json"},
                   {IoType::Io_Type_Bool, IoType::Io_Type_Int, IoType::Io_Type_JSON});
        ...
        if (csv_file_.is_open()) {
            ...
            if (json_data.contains("data")) {
                if (!json_data["data"].empty()) {
                    for (nlohmann::json::iterator it = json_data["data"].at(0).begin();
                         it != json_data["data"].at(0).end(); ++it) {
                        if (!it->is_array()) {
                            csv_file_ << it.key() << ",";
                        } else if (it->is_array()) {
        ...
        if (csv_file_.is_open()) {
            if (limit_file_size_) {
                ...
                if (exists(fPath)) {
                    ...
                    if (fSizeK >= static_cast<unsigned long>(max_file_size_kb_)) {
                        if (rotating_files_) {
                            csv_cur_file_num_++;
                    ...
                    if (!json_in["data"].empty()) {
                        ...
                        for (auto &d : json_in["data"]) {
                            ...
                            if (json_in.contains("ref_frame")) {
                                csv_file_ << json_in["ref_frame"]["w"].get<int>() << "x"
                                          << json_in["ref_frame"]["h"].get<int>() << ",";
                            }
                            ...
                            for (auto &it : d) {
                                ...
                                } else if (it.is_object()) {  // TODO: Handle Deeper Level JSON
                                                              // Objects
                                }
                            }
                            csv_file_ << "\n";
...
void CsvFile::UpdateGui(void *context, int interface)
{
    ...
    if (interface == static_cast<int>(FlowCV::GuiInterfaceType_Controls)) {
        ...
        if (file_dialog_.showFileDialog(CreateControlString("Save CSV", GetInstanceName()),
                                        imgui_addons::ImGuiFileBrowser::DialogMode::SAVE,
                                        ImVec2(700, 310), ".csv", &show_file_dialog_)) {
        ...
        ImGui::Separator();
        bool newFile = false;
        if (ImGui::Checkbox(CreateControlString("Save Timestamp", GetInstanceName()).c_str(),
                            &save_timestamp_)) {
            newFile = true;
        }
        if (ImGui::Checkbox(CreateControlString("Save Index Count", GetInstanceName()).c_str(),
                            &save_counter_)) {
            newFile = true;
        }
        ImGui::SetNextItemWidth(120);
        if (ImGui::Checkbox(CreateControlString("Limit File Size", GetInstanceName()).c_str(),
                            &limit_file_size_)) {
            newFile = true;
        }
        if (limit_file_size_) {
            ImGui::SetNextItemWidth(120);
            ImGui::DragInt(CreateControlString("Max Size KB", GetInstanceName()).c_str(),
                           &max_file_size_kb_, 0.5F, 100, INT_MAX);
            if (ImGui::Checkbox(
                    CreateControlString("Use Rotating Files", GetInstanceName()).c_str(),
                    &rotating_files_)) {
                newFile = true;
            }
            if (rotating_files_) {
                ImGui::SetNextItemWidth(120);
                if (ImGui::DragInt(CreateControlString("Num Files", GetInstanceName()).c_str(),
                                   &num_files_, 0.5F, 2, 100)) {
                    newFile = true;
                }
            }
        }
