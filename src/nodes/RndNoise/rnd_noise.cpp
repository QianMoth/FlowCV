...
void RndNoise::UpdateGui(void *context, int interface)
{
    ...
    if (interface == (int)FlowCV::GuiInterfaceType_Controls) {
        if (ImGui::Combo(CreateControlString("Mask Mode", GetInstanceName()).c_str(), &tmp_mode_,
                         "Color\0BW_8U\0BW_8UC3\0\0"))
            change_settings_ = true;
        ImGui::SetNextItemWidth(100);
        if (ImGui::DragInt(CreateControlString("Width", GetInstanceName()).c_str(), &tmp_width_,
                           0.5f, 1, 8000))
            change_settings_ = true;
        ImGui::SetNextItemWidth(100);
        if (ImGui::DragInt(CreateControlString("Height", GetInstanceName()).c_str(), &tmp_height_,
                           0.5f, 1, 8000))
            change_settings_ = true;
