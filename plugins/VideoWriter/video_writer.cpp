...
{
    static const auto *codec_list =
        new std::vector<std::pair<const std::string, const std::string>>{
            {"H264", ".mp4"}, {"I420", ".mp4"}, {"IYUV", ".mp4"}, {"M4S2", ".mp4"},
            {"MP4S", ".mp4"}, {"MP4V", ".mp4"}, {"MJPG", ".avi"}, {"UYVY", ".avi"},
            {"WMV1", ".wmv"}, {"WMV2", ".wmv"}, {"WMV3", ".wmv"}, {"WMVA", ".wmv"},
            {"WMVP", ".wmv"}, {"WVC1", ".wmv"}, {"WVP2", ".wmv"}, {"YUY2", ".mpg"},
            {"YV12", ".mpg"}, {"YVU9", ".mpg"}, {"YVYU", ".mpg"}};
    return *codec_list;
}
...
VideoWriter::VideoWriter() : Component(ProcessOrder::OutOfOrder), p(new internal::VideoWriter())
{
    ...
    // add 2 inputs
    SetInputCount_(3, {"in", "fps", "start"},
                   {IoType::Io_Type_CvMat, IoType::Io_Type_Int, IoType::Io_Type_Bool});
...
void VideoWriter::SaveSource()
{
    if (!out_filename_.empty()) {
        ...
        int fourcc =
            cv::VideoWriter::fourcc(codec_str_[0], codec_str_[1], codec_str_[2], codec_str_[3]);
        if (auto_ext_) {
        ...
        fps_time_ = (1.0F / static_cast<float>(fps_)) * 1000.0F;
        video_writer_.open(out_filename_, fourcc, static_cast<float>(fps_),
                           cv::Size(frame_.cols, frame_.rows), true);
        save_new_file_ = false;
        last_fps_ = fps_;
        last_codec_ = codec_;
    }
    ...
    if (!in1->empty()) {
        ...
        if (allow_write_) {
            ...
            if (!video_writer_.isOpened()) {
                ...
                // - last_time_).count(); if ((float) delta >= fps_time_) {
    ...
    if (interface == static_cast<int>(FlowCV::GuiInterfaceType_Controls)) {
        ...
        if (ImGui::Combo(
                ...
                [](void *data, int idx, const char **out_text) {
                    *out_text =
                        (static_cast<const std::vector<std::pair<std::string, std::string>> *>(
                             data))
                            ->at(idx)
                            .first.c_str();
                    return true;
                },
                (void *)&FourCCList, static_cast<int>(FourCCList.size()))) {
            codec_str_ = FourCCList[codec_].first;
        }
        if (!has_fps_in_) {
            ImGui::SetNextItemWidth(80);
            const int fpsValues[] = {1, 3, 5, 10, 15, 20, 25, 30, 60, 120};
            if (ImGui::Combo(CreateControlString("Video File FPS", GetInstanceName()).c_str(),
                             &fps_index_, " 1\0 3\0 5\0 10\0 15\0 20\0 25\0 30\0 60\0 120\0\0")) {
                fps_ = fpsValues[fps_index_];
            }
        }
        ImGui::Checkbox(CreateControlString("Auto File Extension", GetInstanceName()).c_str(),
                        &auto_ext_);
        ...
        if (file_dialog_.showFileDialog(CreateControlString("Save Movie", GetInstanceName()),
                                        imgui_addons::ImGuiFileBrowser::DialogMode::SAVE,
                                        ImVec2(700, 310), ".avi,.mpg,.mp4,.mjpg,.mkv,.wmv",
                                        &show_file_dialog_)) {
            std::string outExt = FourCCList[codec_].second;
