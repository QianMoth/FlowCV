...
{
    const std::vector<const char *> backends = {"Default", "Halide", "Inference Engine",
                                                "OpenCV",  "VkCom",  "Cuda"};
    ...
    for (const auto &b : backendList) {
        if ((int)b.first == 1000000 || b.first == cv::dnn::DNN_BACKEND_INFERENCE_ENGINE) {
            auto i = std::find_if(backend_list_.begin(), backend_list_.end(),
                                  backendListCompare(backends[2]));
            ...
            if (b.first < backends.size()) {
                auto i = std::find_if(backend_list_.begin(), backend_list_.end(),
                                      backendListCompare(backends[b.first]));
                if (i == backend_list_.end()) {
...
{
    const std::vector<const char *> targets = {"CPU",  "OpenCL", "OpenCL FP16", "Myriad", "Vulkan",
                                               "FPGA", "CUDA",   "CUDA FP16",   "HDDL"};
